...
\begin{itemize}
\item
The \texttt{gl} subdirectory of the \texttt{810f22} git repository has a file \texttt{test.sbat} which submits a batch job running the parallel \texttt{foreach} test in \texttt{test.R}.
\item
...
<<f12,echo=T,eval=F>>=
git clone https://github.com/ionides/810f22.git
@
\end{itemize}
...
\begin{frame}{Acknowledgments}
  \begin{itemize}
  \item
    This lesson is prepared for STATS 810, Fall 2022.
  \item
